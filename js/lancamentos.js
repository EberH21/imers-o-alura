...
let nextPage = 1;
const ORDERING = "name"; // Ordena por nome (ordem alfabética)
...
/**
 * Busca jogos na API da RAWG ordenados por lançamento.
 */
async function buscarJogos(page = 1) {
    try {
        ...
        const dataInicial = `${anoAtual}-${mesAtual}-${diaAtual}`;
        const dataFinal = `${anoAtual + 5}-12-31`; // Busca por lançamentos nos próximos 5 anos

        const url = `https://api.rawg.io/api/games?key=${API_KEY}&dates=${dataInicial},${dataFinal}&ordering=${ORDERING}&page=${page}`;
